...
author_manual = {}
with open('data/author-manual-info.csv', newline='', encoding='utf-8') as f:
...
#once the gender dates are manually completed, then update the author info with other fields
...
def get_author_info(author_id):
    author_obj = goodreads.get_object('author', author_id)
    author_info = dict(author_id=author_id)
    dict_fields = ['name', 'image_url', 'link' ,'gender', 'born_at', 'died_at']
    for field in dict_fields:
        author_info[field] = author_obj[field]
...
# get author ids
author_ids = []
with open('data/fiction_goodreads-ids.csv', newline='', encoding='utf-8') as f:
    for row in csv.DictReader(f):
...
already_parsed = []
with open('data/author-info.csv', newline='', encoding='utf-8') as f:
    for row in csv.DictReader(f):
